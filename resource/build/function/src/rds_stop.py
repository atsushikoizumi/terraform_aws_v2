    ...
    for i in range(len(res['Reservations'])):
        ...
        for j in range(len(res['Reservations'][i]['Instances'][0]['Tags'])):
            if str(res['Reservations'][i]['Instances'][0]['Tags'][j]['Key']) == 'Env':
                if  str(res['Reservations'][i]['Instances'][0]['Tags'][j]['Value']) == 'dev': # os.environ['tags_env']
                    f += 1
            if str(res['Reservations'][i]['Instances'][0]['Tags'][j]['Key']) == 'Owner':
                if  str(res['Reservations'][i]['Instances'][0]['Tags'][j]['Value']) == 'koizumi': # os.environ['tags_owner']
                    f += 1
            if str(res['Reservations'][i]['Instances'][0]['Tags'][j]['Key']) == 'Name':
                ...
                if  str(res['Reservations'][i]['Instances'][0]['Tags'][j]['Value']) == 'koizumi-dev-win2019': # os.environ['ec2_win_name']
                    f += 1
        if f != 3:
            continue
